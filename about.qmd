...
# Buat data
data <- data.frame(kategori = c("Kategori 1", "Kategori 2", "Kategori 3"), nilai = c(10, 20, 30))

# Buat diagram lingkaran
ggplot(data, aes(x = "", y = nilai, fill = kategori)) +
  geom_pie(alpha = 0.7) +
  theme_minimal()

# Masukkan grafik ke dalam index.qmd
embed_qplot(ggplot(data, aes(x = "", y = nilai, fill = kategori)) +
  geom_pie(alpha = 0.7) +
  theme_minimal())